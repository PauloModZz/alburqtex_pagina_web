...
export default function CommentsSection({ onRequireAuth }: CommentsSectionProps) {
  ...
  const handlePhotoChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith('image/')) {
      setError(isEnglish ? 'Choose an image file.' : 'Elige un archivo de imagen.');
      return;
    }
    if (file.size > MAX_PHOTO_BYTES) {
      setError(isEnglish ? 'The photo cannot exceed 10 MB.' : 'La foto no puede pesar más de 10 MB.');
      return;
    }
    setError(null);
    setPhoto(file);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (rating < 1) {
      setError(isEnglish ? 'Choose a star rating.' : 'Elige una calificación de estrellas.');
      return;
    }
    const trimmed = text.trim();
    if (!trimmed) {
      setError(isEnglish ? 'Write your review.' : 'Escribe tu comentario.');
      return;
    }
    if (trimmed.length > MAX_LENGTH) {
      setError(isEnglish ? `Maximum ${MAX_LENGTH} characters.` : `Máximo ${MAX_LENGTH} caracteres.`);
      return;
    }
    if (LINK_PATTERN.test(trimmed)) {
      setError(isEnglish ? 'Links are not allowed in reviews.' : 'No se permiten enlaces en los comentarios.');
      return;
    }
    ...
    try {
      ...
      const displayName = user.displayName || (isEnglish ? 'Alburqtex customer' : 'Cliente Alburqtex');
      ...
      setError(isEnglish ? 'We could not send your review. Please try again.' : 'No se pudo enviar tu comentario. Intenta de nuevo.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    >
      <div className="max-w-5xl mx-auto px-4 sm:px-8 py-14 sm:py-20">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mt-3 mb-8">
          ...
          >
            {isEnglish ? 'What our customers say' : 'Lo que dicen nuestros clientes'}
          </h2>
          {comments.length > 0 && (
            <div className="flex items-center gap-2">
              <StarRating value={Math.round(average)} size={16} />
              <span className="text-sm text-black/50">
                {average.toFixed(1)} · {comments.length} {isEnglish ? `review${comments.length === 1 ? '' : 's'}` : `comentario${comments.length === 1 ? '' : 's'}`}
              </span>
            </div>
          )}
        </div>
        ...
        {!loading && comments.length === 0 && (
          <p className="text-sm text-black/40 mb-10">{isEnglish ? 'There are no reviews yet — be the first to leave one!' : 'Todavía no hay comentarios — ¡sé el primero en dejar uno!'}</p>
        )}

        <div className="max-w-md rounded-2xl p-6 bg-white border" style={{ borderColor: 'rgba(0,0,0,0.06)' }}>
          {sent ? (
            <p className="text-sm text-black/70">
              {isEnglish ? 'Thank you for your review! We will check it before publishing.' : '¡Gracias por tu comentario! Lo revisamos y aparecerá publicado en unos minutos.'}
            </p>
          ) : !user ? (
            <>
              <p className="text-sm text-black/60 mb-4">{isEnglish ? 'Sign in to leave a review.' : 'Inicia sesión para dejar tu comentario.'}</p>
              <button
                type="button"
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-3.5">
              <p className="text-xs font-semibold uppercase tracking-widest text-black/50">{isEnglish ? 'Your rating' : 'Tu calificación'}</p>
              ...
              <textarea
                ...
                placeholder={isEnglish ? 'Tell us about your experience...' : 'Cuéntanos cómo fue tu experiencia...'}
              ...
              {photoPreview ? (
                <div className="relative w-24 h-24">
                  ...
                  <button
                    type="button"
                    onClick={removePhoto}
                    aria-label={isEnglish ? 'Remove photo' : 'Quitar foto'}
                ...
                >
                  ...
                  {isEnglish ? 'Add a photo (optional)' : 'Agregar una foto (opcional)'}
                </button>
              )}
              <input
                ref={fileInputRef}
              ...
              <button
                ...
                style={{ backgroundColor: '#141414' }}
              >
                {submitting ? (isEnglish ? 'Sending...' : 'Enviando...') : (isEnglish ? 'Send review' : 'Enviar comentario')}
              </button>
            </form>
          )}
        </div>
